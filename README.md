...
| 金鑰 | **每把金鑰一行**：`v{N} (使用中/已退役)`（多版專案會展開成多行儲存格） | 一眼掌握全部金鑰版本與狀態 |
| **私鑰檔** ✓ 存在 / ✗ 遺失 | 對 `keys.private_key_path` **逐版**做 `Path.is_file()` 檢查 | **DB 紀錄路徑與磁碟實際狀況不一致時立即發現**（最常見：路徑被搬走、檔案被刪、誤連到別台機器的舊路徑） |
| 公鑰指紋(前16) | `keys.public_key_fp[:16]`，多版時每版一行 | 比對你手上的 `.lic` / `verify_license.py` 是否來自同一把 |
| 授權 | `len(licenses) (有效/撤銷)` | 看出哪些專案還活著 |

`✗ 遺失` 出現時，可進「🔁 DB 維運 → 修復金鑰路徑」用 auto-relink 自動修——它會在 keys_dir 內找符合檔名的候選 `.pem`，**衍生其公鑰並與 DB 紀錄的 `public_key_pem` 位元級比對**，只有真正屬於該專案的金鑰會被連結（檔名碰撞但密鑰不同的檔案會被跳過，避免靜默誤連）。

### 🔎 專案詳情（深入單一專案）

主選單 → `📁 專案管理` → `🔎 查看專案詳情` 可挑一個專案，展開：
- **Metadata panel**：專案 ID / 名稱 / 版本 / 環境前綴 / 指紋版本 / 有效天數 / 建立日 + 可選的 Git 來源資訊。
- **🔑 金鑰表**：所有金鑰版本（包含退役者），含完整 `private_key_path`、實際檔案是否存在、公鑰指紋（前 32）、建立日、狀態。
- **📄 授權電腦表**：每筆授權的**完整 64-hex 機器指紋**、簽發時用的 Key 版本、客戶名、簽發/到期日、可選的 MAC 審計提示、有效/撤銷狀態。
