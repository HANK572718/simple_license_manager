...
from rich_deploy.core.db.crud import create_key, get_active_key, get_project, list_keys
from rich_deploy.core.db.engine import RICH_DEPLOY_DATA_DIR, init_db
from rich_deploy.core.generate_keys import generate_key_pair
...
_DEFAULT_KEYS_ROOT = RICH_DEPLOY_DATA_DIR / "projects"
...
class KeyGenerateCommand(Command):
    ...
    options = [
        option(
            "--keys-dir",
            None,
            "Override the root directory for key storage (default: ~/.ssh/rich_deploy/projects)",
            flag=False,
            default=None,
        ),
    ]
    ...
    def handle(self) -> int:
        ...
        self.line("<comment>Private key stored in ~/.ssh/rich_deploy/ — never commit it.</comment>")
        return 0
